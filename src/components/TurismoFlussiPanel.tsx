    ...
    <div className="mt-4 panel overflow-hidden p-0">
      <div className="border-b border-[var(--pa-border)] bg-[var(--pa-surface-soft)] px-4 py-3 sm:px-5">
        <PanelHeading
          ...
          description={t("Arrivi e presenze comunali da Regione Toscana (ISTAT)")}
          icon={Palmtree}
          className="mb-0"
        />
      </div>
      ...
      <div className="px-4 py-4 sm:px-5 sm:py-5">
        <PanelState
          state={state}
          title={t("Flussi turistici non disponibili")}
          emptyHint={t(
          ...
          loadingLabel={t("Caricamento flussi turistici…")}
        >
          {(data) => {
            const anno = data.anno;
            const annoPrec = data.annoPrecedente;
            const curPresenze =
              data.annuale.find((a) => a.anno === anno)?.presenze ??
              data.mensile.reduce((s, m) => s + m.presenze, 0);
            const curArrivi =
              data.annuale.find((a) => a.anno === anno)?.arrivi ??
                  ...
                  {hasProvenienza ? (
                    <div>
                      ...
                      <DoughnutChart
                        ...
                        values={[
                          data.provenienza.italiani,
                          data.provenienza.stranieri,
                        ]}
                      />
